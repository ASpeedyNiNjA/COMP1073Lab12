...
h24jW2Ut9Pks3rcsJnnuK9v7nYRhLT21iZ1ZfMky
...
const user = "h24jW2Ut9Pks3rcsJnnuK9v7nYRhLT21iZ1ZfMky";
...
hueSlider.addEventListener("change", function() {
	...
	let newSat = this.value * 6;
	updateScreenColor(newHue);
	let commands = '{ "hue" : ' + newHue + ', "sat" : ' + newSat + ', "bri" : 100, "on" : true }';
	// STEP 9b: Invoke the completed updateLight() function when the hueSlider value changes
	updateLight(commands);
}, false);

// Function that change the page saturation based on the value of the sat
// function updateScreenSat(newSat){
...
async function updateLight(commands) {
	...
	let response = await fetch(url, {
		...
		mode: "cors",  //cross-origin resource sharing without browser getting upset
		body: commands
	}).then(response => response.json()) //convert response into json because it will come back as text
	.then(response => console.log("Success: ", response))
	.catch(error => console.warn("Error:", error)); //console.warn is similar to console.log
